...
int main(int argc, char *argv[]) {
  if (argc != 3) {
    ...
    if (file == NULL) {
      ...
      char line[MAXLINE]; /* lines of file will be read into line; line will be
                             searched for occurrences */
      char fullLine[MAXLINE]; /* before line is modified, it will be copied to
                                 fullLine */
      char *delim = " .,?!;:()\"\n";
      int lineNum = 0;
      ...
      while (fgets(line, MAXLINE, file) != NULL) {
        ...
        while (ptr) {
          if (!strcmp(ptr, argv[1])) {
            ...
            if (!head) /* If the first node of the linked list has been created,
                          it is set as the head*/
              head = tail;
            occurrenceCount++;
          }
      ...
      printf("Longest line (%d characters): %s", longestLineNum,
             longestLineStr);
